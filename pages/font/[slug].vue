...
<template>
  <div class="flex-col text-4xl mx-4 sm:mx-10 overflow-visible py-2 justify-between text-[#1a1c1e] space-y-8">
    ...
    <div class="flex justify-between items-center dark:text-[#ffff]">
      ...
      <a :href="data.link">
        <p class="hover:scale-105 ease-in-out duration-500 flex items-center bg-[#1c2c28] text-[#ffff] p-2 mx-1.5 py-1 rounded-3xl"
          style="direction: ltr;">
    ...
    <div class="dark:text-[#ffff] leading-loose flex justify-center pb-10 text-center"
      :style="{ fontFamily: data.family, fontSize: data.size + 10 + 'px' }">اردو ہے جس کا نام ہمیں جانتے ہیں داغ​</div>

    <div class="flex-row sm:flex-col my-12 mx-2 items-center space-y-6">
      <div class="sm:w-1/2 flex bg-green-50 dark:bg-[#1c2c28] p-1 rounded-full mb-12"><input type="text" v-model="text"
      ...
      <div class="flex items-center w-full sm:w-[80%]">
        ...
        <div class="inter text-xl rounded-3xl bg-[#1c2c28] p-3 py-1 text-[#ffff] inline-block">{{ value }}px</div>
      </div>
    ...
    <div v-for="(style) in data.styles" :key="style" v-if="data.styles.length >= 1">
      ...
      <div class="mt-10 mb-8 leading-relaxed dark:text-[#ffff]"
        :style="{ fontSize: value + 'px', fontWeight: style.split(' ')[1], fontFamily: data.family }">{{ text.length ==
