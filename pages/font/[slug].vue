...
// Add missing reactive variables for template
const text = ref("");
const value = ref(data.size);
...
<template>
	<div class="flex-col text-4xl mx-4 sm:mx-10 overflow-visible py-2 justify-between text-[#1a1c1e] space-y-8">
		...
		<div class="flex justify-between items-center dark:text-[#ffff]">
			...
			<a :href="data.link">
				<p class="hover:scale-105 ease-in-out duration-500 flex items-center bg-[#1c2c28] text-[#ffff] p-1.5 mx-1.5 py-0.5 rounded-none"
					style="direction: ltr;">
					<Icon name="lucide:arrow-down" size="24px"></Icon>
					<div class="inter relative px-1 text-[19px] mehr">Download</div>
				</p>
			</a>

		</div>

		<div class="dark:text-[#ffff] leading-loose flex justify-center pb-10 text-center"
			:style="{ fontFamily: data.family, fontSize: data.size + 10 + 'px' }">اردو ہے جس کا نام ہمیں جانتے ہیں داغ​
		</div>

		<div class="flex-row sm:flex-col my-12 mx-2 items-center space-y-6">
			<div class="sm:w-1/2 flex bg-green-50 dark:bg-[#1c2c28] p-1 rounded-full mb-12"><input type="text"
					v-model="text"
					class="w-full mr-4 mb-1 leading-[4rem] overflow-visible noto-nastaliq text-[20px] bg-transparent focus:outline-none focus:ring-0 placeholder:text-[#1a1c1e] dark:placeholder:text-[#ffff]"
					placeholder="الفاظ  یہاں تحریر کریں">
			</div>

			<div class="flex items-center w-full sm:w-[80%]">
				<URange v-model="value" :min="10" :max="99" class="mx-6 sm:w-1/2" :ui="{
				...
				<div class="inter text-xl rounded-3xl bg-[#1c2c28] p-3 py-1 text-[#ffff] inline-block">{{ value }}px
				</div>
			</div>
		...
		<div v-for="(style) in data.styles" :key="style" v-if="data.styles.length >= 1">
			...
			<div class="mt-10 mb-8 leading-relaxed dark:text-[#ffff]"
				:style="{ fontSize: value + 'px', fontWeight: style.split(' ')[1], fontFamily: data.family }">{{
					text.length ==
						0 ? data.urdu + ' اردو کا ایک فونٹ ہے' : text }}</div>
		</div>
